...
def find_latest_transcript() -> Optional[str]:
    ...
    Falls back to the live OpenCode session when the legacy vendor
    ``session-env`` JSONL files are absent (OpenCode never writes those).
    All fallback calls are guarded so a missing DB, missing ``opencode``
    binary, or a non-zero exit degrades to ``None`` rather than hanging.
    """
    try:
        if os.getenv('TRANSCRIPT_PATH'):
            return os.getenv('TRANSCRIPT_PATH')

        session_dir = Path(os.getenv('SESSION_DIR', Path.home() / '.opencode' / 'session-env')).expanduser()
    ...
    # Fall back to the live OpenCode session export.
    try:
        from opencode_session import current_session_id, opencode_export_transcript
        sid = current_session_id()
        if sid:
            exported = opencode_export_transcript(sid)
            if exported:
                return exported
    except Exception:
        pass
...
def load_transcript(path: str) -> List[Dict[str, Any]]:
    """Load a transcript file into a message list.

    Supports two formats:
      * JSONL (one JSON object per line) -- the legacy vendor transcript.
      * OpenCode export JSON -- a single JSON document whose top level is an
        object with a ``messages`` key (or a bare array of messages). Detected
        by attempting to parse the whole file as a single JSON value; if that
        succeeds and the shape matches an export, it is normalised via
        ``load_opencode_export``. A JSONL file does not parse as a single
        document, so it falls through to the line-by-line path.
    """
    try:
        with open(path) as f:
            text = f.read()
    ...
    if isinstance(parsed, dict) and 'messages' in parsed:
        return load_opencode_export(parsed)
    if isinstance(parsed, list):
        # Top-level array of messages: wrap and normalise.
        return load_opencode_export({'messages': parsed})
...
def load_opencode_export(data: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Normalise an OpenCode export document into message dicts.

    OpenCode export shape::
    ...
    Returns a list of ``{"role", "content", "tool_uses"}`` dicts matching what
    the rest of the code expects. Non user/assistant messages are skipped.
    """
    messages = []
    raw_messages = (data or {}).get('messages', []) if isinstance(data, dict) else []
    for m in raw_messages:
        ...
        for part in parts:
            ...
            if ptype == 'text':
                ...
                parameters = state.get('input', {}) if isinstance(state, dict) else {}
                tool_uses.append({'name': name, 'parameters': parameters})
    ...
    for msg in messages:
        if 'tool_uses' in msg:
            for tool in msg.get('tool_uses', []):
                ...
                # Existing: vendor tool named exactly 'Skill' with a 'skill' param.
                if name == 'Skill' and 'skill' in params:
                    skills.add(str(params['skill']))
                # Extended: OpenCode's case-insensitive 'skill' tool, with either
                # a 'skill' or a 'name' parameter carrying the skill id.
                if name.lower() == 'skill':
                    for key in ('skill', 'name'):
                        if key in params and params[key]:
                            skills.add(str(params[key]))
...
def extract_approach(message: Dict[str, Any]) -> str:
    """Extract the approach the assistant took from the assistant message"""
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Extract learning signals from coding-session transcripts'
    )
    parser.add_argument(
        'transcript',
        nargs='?',
