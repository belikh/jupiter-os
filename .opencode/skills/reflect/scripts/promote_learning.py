#!/usr/bin/env python3
"""
Promote Learning - Move skill-level learnings to global AGENTS.md

When a learning appears in multiple repositories, it becomes a candidate
for promotion to the global AGENTS.md file.
"""
...
# Paths
GLOBAL_RULES_MD = Path.home() / ".opencode" / "AGENTS.md"
BACKUP_DIR = Path.home() / ".opencode" / "backups"
...
class LearningPromoter:
    ...
    def preview_promotion(self, fingerprint: str) -> Dict:
        """Preview what would be added to AGENTS.md."""
        ...
        # Generate the entry for AGENTS.md
        ...
        return {
            ...
            "target_file": str(GLOBAL_RULES_MD)
        }

    def _format_entry(self, learning: Dict) -> str:
        """Format learning as AGENTS.md entry."""
        content = learning['content']
    ...
    def promote(self, fingerprint: str, dry_run: bool = False) -> Dict:
        """Promote a learning to global AGENTS.md."""
        learning = self.ledger.get_learning(fingerprint)
        if not learning:
        ...
        if dry_run:
            return {
                ...
                "dry_run": True,
                "would_add": entry,
                "to_file": str(GLOBAL_RULES_MD)
            }

        # Backup existing file
        backup_path = self._backup_file(GLOBAL_RULES_MD)

        # Append to AGENTS.md
        GLOBAL_RULES_MD.parent.mkdir(parents=True, exist_ok=True)
        
        with open(GLOBAL_RULES_MD, 'a') as f:
            f.write(entry)
