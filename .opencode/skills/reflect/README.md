...
Ein intelligentes Lernsystem für your AI coding assistant, das aus Ihren Korrekturen lernt und Skills automatisch verbessert.
...
Semantic Detection nutzt the assistant selbst als ML-Engine für intelligentere Pattern-Erkennung:
...
Learnings werden über Skills und Repositories hinweg getrackt. Wenn ein Learning in 2+ Repos auftaucht, kann es zu deiner globalen `~/.opencode/AGENTS.md` promoted werden.
...
Repo B: "Verwende uv statt pip" → Gleiche Learning erkannt!
    ...
    Threshold erreicht (2 Repos)
        ↓
    /reflect-promote → Global AGENTS.md
        ↓
    the assistant weiß es ÜBERALL ✨
```
...
python3 ~/.opencode/skills/reflect/scripts/promote_learning.py list

# Preview einer Promotion
python3 ~/.opencode/skills/reflect/scripts/promote_learning.py preview <fingerprint>

# Learning promoten
python3 ~/.opencode/skills/reflect/scripts/promote_learning.py promote <fingerprint>

# Alle eligible Learnings promoten
python3 ~/.opencode/skills/reflect/scripts/promote_learning.py all --dry-run
```
...
| Datei | Beschreibung |
|-------|--------------|
| `~/.opencode/reflect/learnings.db` | SQLite Ledger |
| `~/.opencode/AGENTS.md` | Globale Regeln |
| `~/.opencode/backups/` | Automatische Backups |

---

## 📖 Was ist Reflect?

Reflect analysiert Ihre Konversationen mit the assistant und:

- ✅ **Erkennt Korrekturen** - Wenn Sie the assistant korrigieren
...
Session 2: the assistant verwendet uv ✅
Session 3: the assistant verwendet uv ✅
Session N: the assistant verwendet uv ✅
```
...
| Backup nötig | `~/.opencode/backups/` prüfen |
...
Entwickelt für your AI coding assistant mit ❤️
