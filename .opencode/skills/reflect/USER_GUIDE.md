...
*"Correct once, never again"* - Ein System, das aus Ihren Korrekturen lernt und your AI coding assistant kontinuierlich verbessert.
...
Reflect ist ein intelligentes Lernsystem für your AI coding assistant, das:

- **Korrekturen erkennt** wenn Sie the assistant korrigieren
...
Normalerweise wiederholt the assistant dieselben Fehler in jeder neuen Session:

```
Session 1: the assistant verwendet pip
Sie:      "Nein, verwende uv statt pip"
the assistant:   "Ok, ich verwende jetzt uv"

Session 2: the assistant verwendet wieder pip 😞
Sie:      "Ich hab dir doch gesagt, verwende uv!"
```

### Die Lösung

Mit Reflect lernt the assistant dauerhaft:

```
Session 1: the assistant verwendet pip
Sie:      "Nein, verwende uv statt pip"
the assistant:   Führt /reflect aus → Aktualisiert Python-Skill

Session 2: the assistant verwendet automatisch uv ✅
Session 3: the assistant verwendet automatisch uv ✅
Session N: the assistant verwendet automatisch uv ✅
```
...
Nachdem Sie the assistant in einer Session korrigiert haben:
...
**Wann verwenden?**
- Sie haben the assistant gerade korrigiert
...
**Workflow:**
1. Sie arbeiten mit the assistant und korrigieren Fehler
...
**Wann verwenden?**
- Sie arbeiten täglich intensiv mit the assistant
...
**Workflow:**
1. Sie arbeiten ganz normal mit the assistant
...
the assistant: Ich verwende pip install für die Dependencies...
*verwendet pip*

User: Nein, verwende bitte uv statt pip. Uv ist schneller und moderner.

the assistant: Verstanden, ich verwende jetzt uv install...
*verwendet uv*
```
...
**Ergebnis:** Ab jetzt verwendet the assistant automatisch `uv` statt `pip` in Python-Projekten!
...
the assistant: Ich strukturiere die Tests mit pytest in folgende Kategorien:
...
the assistant: Wie möchten Sie die Error-Handling verbessern?

User: Have you considered using custom exception classes statt generic try-catch blocks?

the assistant: Gute Idee! Das würde die Fehler spezifischer machen.
```
...
4. **Nicht Backups löschen**
   - Backups liegen in `~/.opencode/skills/{skill-name}/.backups/`
...
3. **Transcript nicht gefunden**
   - Lösung: Prüfen Sie `~/.opencode/session-env/*/transcript.jsonl`
...
1. **Skill-Datei korrupt**
   - Prüfen: `cat ~/.opencode/skills/{skill-name}/SKILL.md`
   - Lösung: Aus Backup wiederherstellen:
     ```bash
     cp ~/.opencode/skills/{skill-name}/.backups/SKILL_*.md \
        ~/.opencode/skills/{skill-name}/SKILL.md
     ```

2. **Keine Schreibrechte**
   - Prüfen: `ls -la ~/.opencode/skills/{skill-name}/`
   - Lösung: `chmod -R u+w ~/.opencode/skills/`
...
2. **Hook konfiguriert?**
   ```bash
   cat ~/.opencode/settings.local.json | grep -A 10 hooks
   ```
   Sollte enthalten:
   ```json
   "hooks": {
...
3. **Script ausführbar?**
   ```bash
   ls -la ~/.opencode/skills/reflect/scripts/hook-stop.sh
   # Sollte -rwxr-xr-x zeigen
   ```

4. **Log prüfen:**
   ```bash
   cat ~/.opencode/reflect-hook.log
   ```
...
```bash
cd ~/.opencode/skills/{skill-name}/.backups
...
```bash
~/.opencode/skills/reflect/scripts/extract_signals.py
```
...
```bash
~/.opencode/skills/reflect/references/signal-patterns.md
```
...
- Learnings gelten für **alle** your AI coding assistant Sessions
- Unabhängig vom Projekt-Directory
- Skills in `~/.opencode/skills/` sind global

**Pro-Projekt-Skills:**
Erstellen Sie projekt-spezifische Skills in `.opencode/skills/` (lokal).
...
- Check Log: `tail ~/.opencode/reflect-hook.log`
...
nano ~/.opencode/skills/reflect/scripts/extract_signals.py
```
...
nano ~/.opencode/skills/reflect/scripts/reflect.py
```
...
2. **Scripts:**
   ```
   ~/.opencode/skills/reflect/scripts/extract_signals.py    # Pattern-Detection
   ~/.opencode/skills/reflect/scripts/update_skill.py       # Skill-Updates
   ~/.opencode/skills/reflect/scripts/present_review.py     # Review-UI
   ```
...
TRANSCRIPT_PATH=/path/to/transcript.jsonl \
  python3 ~/.opencode/skills/reflect/scripts/reflect.py
```

**Log-Datei prüfen:**

```bash
tail -f ~/.opencode/reflect-hook.log
```
...
```
~/.opencode/skills/reflect/              # Reflect Skill
~/.opencode/skills/reflect/.state/       # Status & Locks
~/.opencode/skills/{skill}/.backups/     # Backups (30 Tage)
~/.opencode/settings.local.json          # Hook-Konfiguration
~/.opencode/reflect-hook.log             # Hook-Logs
```
