...
> **OpenCode setup:** Skills resolve via the `SKILLS_ROOT` env var (default
> `~/.opencode/skills`, auto-detecting `<cwd>/.opencode/skills` — e.g. this
> project's `jupiter-os/.opencode/skills`). No `PyYAML` needed (a tiny vendored
> `yaml_min` is used). `python3` + `git` should be on PATH. Auto-reflection is
> OFF by default. To run manually:
> ```bash
> SID=$(python3 .opencode/skills/reflect/scripts/opencode_session.py)
> opencode export "$SID" > /tmp/reflect_transcript.json
> SKILLS_ROOT=.opencode/skills TRANSCRIPT_PATH=/tmp/reflect_transcript.json \
>   python3 .opencode/skills/reflect/scripts/reflect.py --non-interactive
> ```
> `--non-interactive` auto-applies HIGH-confidence corrections (override with
> `REFLECT_CONFIDENCE_MIN`) and writes a run log under `reflect/.state/reflect_runs/`.

## Overview

This skill enables your AI coding assistant to learn from conversations by analyzing corrections,
...
- User contradicts the assistant's approach with specific alternative
...
5. **User Review** - Interactive approval, or `--non-interactive` which auto-applies HIGH-confidence corrections and logs the rest
