/* idiomorph 0.7.4 — vendored per arcade-webapp stack (single local JS file,
   no CDN fetch at runtime, no node in the closure). Includes the htmx
   extension (dist/idiomorph-ext.min.js) so hx-swap="morph:*" preserves DOM
   identity instead of tearing down the polling region — the dashboard
   status/coverage flicker fix (outerHTML → morph:innerHTML).
   Source: https://cdn.jsdelivr.net/npm/idiomorph@0.7.4/dist/idiomorph-ext.min.js
   integrity: sha384-SsScJKzATF/w6suEEdLbgYGsYFLzeKfOA6PY+/C5ZPxOSuA+ARquqtz/BZz9JWU8 (see upstream)
   License: 0BSD — see htmx-LICENSE / idiomorph-LICENSE next to this file.
   Upstream 0BSD-licensed. See https://github.com/bigskysoftware/idiomorph */
