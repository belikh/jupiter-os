# NixOS module for OpenDesign — vendored from nexu-io/open-design
# nix/nixos.nix at db9ebb7b, the last commit before the official Nix
# distribution was retired (49cc5105 removed flake.nix and nix/). Upstream
# no longer ships or maintains this; jupiter-os owns it now.
#
# Only difference from upstream: `moduleCommon` is imported directly instead
# of receiving the flake reference, so `services.open-design.package` and
# `services.open-design.webFrontend.package` have no flake-derived defaults.
# jupiter-os injects the vendored packages from pkgs/open-design-{daemon,web}
# in flake.nix.
#
# Usage:
...
#     package = <open-design-daemon>;
#     webFrontend.package = <open-design-web>;
#   };
{
  config,
  lib,
...
let
  ...
  # The static SPA calls `/api/*`, `/artifacts/*`, `/frames/*` at the same
  # origin; caddy proxies those to the daemon. SSE on `/api/*` requires no
  # buffering (flush_interval, no encode).
