# Shared option definitions for the vendored OpenDesign NixOS module.
#
# Vendored from nexu-io/open-design nix/module-common.nix at db9ebb7b — the
# last commit before the official Nix distribution was retired (49cc5105).
# Only difference from upstream: there is no flake input any more, so the
# `package` and `webFrontend.package` defaults cannot point at
# `flake.packages.<system>`; jupiter-os injects the vendored packages from
# pkgs/open-design-{daemon,web} via the services.open-design.package /
# services.open-design.webFrontend.package options instead.
{
  lib,
  pkgs,
  defaultDataDir,
}:
{
  ...
  package = lib.mkOption {
    ...
    default = throw ''
      open-design: services.open-design.package was not set.

      Upstream retired its Nix distribution, so there is no flake default any
      more. jupiter-os injects the vendored daemon package (pkgs/open-design-daemon)
      through its service module; set the option explicitly if you are wiring
      this module up yourself.
    '';
  ...
  webFrontend = {
    ...
    package = lib.mkOption {
      type = lib.types.package;
      default = throw ''
        open-design: services.open-design.webFrontend.package was not set.

        Upstream retired its Nix distribution, so there is no flake default any
        more. jupiter-os injects the vendored web package (pkgs/open-design-web)
        through its service module; set the option explicitly if you are wiring
        this module up yourself.
      '';
