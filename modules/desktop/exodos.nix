...
in
{
  imports = [
    ...
    # (kiosks) and arcade-console.nix (boot-to-arcade appliances) declare the
    # jupiter-arcade session mode this module hooks. Importing them here
    # keeps this module eval-safe regardless of import order in the host
    # (NixOS dedups module imports by path).
    ./arcade.nix
  ...
  config = lib.mkIf cfg.enable {
    ...
    # modules/desktop/dashboard-gaming.nix's `arcade` modeSpecs entry on
    # kiosks, or by modules/desktop/arcade-console.nix on boot-to-arcade
