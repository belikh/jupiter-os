...
- **Phase:** 4 complete, 5 in progress — **P1–P8 all won blind** · final full-PR audit → PR to main
...
| P8 — eXo integration + sprawl retirement | **won** — 1 loop, adversarial audit verdict ship (zero HIGH/MED; 2 LOW residuals recorded post-audit — ADV-P8-01/02 below; sweep incident proven clean: all 8 accidentally-swept WIP files restored byte-identical, blob-audited no secrets) | 1 (VM runs: 2 FAILs — one smoke-side escaping defect + one latent -race data race, both root-caused below — then runs 3+4 = two consecutive clean PASSes) + the P7 critic carry-in landed first (`3add958`) | **ours** (blind; A=anonymized RomM docker-compose, B=our retirement diff + live library DOM with eXo dos games + /inventory.json parity output) | evidence-only — local capture ran no verify pass (cards `data-verify=unknown`); VM smoke owns verify parity proof, noted accepted-residual | commits `3add958` `cd74023` `d95e1a1` `7a44717`(+repair `9a36787`) + this commit; eXo import read-only via internal/exo, /inventory.json parity pinned vs legacy fixture, repo-side sprawl retirement (5 units removed from flake, 3 scripts → scripts/deprecated/); `p8-ours-coherence/`, `p8-bar-compose.yml`; critic: "A's own compose file admits it can never replace the operator's six tools… while adding a debug-shell container, duplicate databases, and an auth stack to babysit" |
...
| ADV-P7-06 LOW — name/collection inputs carry client-side `maxlength` only; the server accepts arbitrary lengths | **accepted** (residual, recorded here — final audit found no record) | LAN-trust posture consistent with accepted ADV-P1-07/08 (cookie-less LAN-only operator surface; the X-HX-Request CSRF check is the proportionate control); stored names render through html/template escaping, and the generator's sanitizeValue bounds what an oversized/hostile name can do to the file trees (no structure injection into metadata files) |
...
### Final audit residuals (ADV-P8-01/02 — recorded pre-merge)

The final full-PR audit found two LOW findings that had no ledger rows.
Recorded here with their dispositions; both are follow-up-after-merge.

| Finding | Disposition |
|---|---|
| ADV-P8-01 LOW — `UpsertSystems`/`UpsertExoSystem` upsert `ON CONFLICT(key)` and overwrite the `source` column, so a NEW catalogue key colliding with an eXo bucket key (`dos`/`win3x`/`win9x`) silently converts the row catalogue↔exo on alternating scans/imports — a self-healing flip-flop (games rows survive; only provenance/bucket metadata lies between passes), but silent | **follow-up-after-merge** — add an import-time guard mirroring `ReservedShortnameError`: probe a new catalogue key against the exo-sourced slice and reject/warn loudly instead of flipping |
| ADV-P8-02 LOW — stale doc comments still citing deleted units as live: `modules/services/arcade-webapp.nix`'s `aria2SecretFile` doc claimed declaration by `jupiter.services.romAcquire`, and `scripts/aria2-rpc.sh`'s header named `modules/services/rom-acquire.nix` as its caller | both load-bearing instances fixed this commit (the option doc now names `jupiter.services.aria2` + D-P2b's pointer-only rule; the script header names internal/aria2 as the porting consumer + scripts/deprecated/ as the retirement record); remaining historical mentions accurately say "retired" — repo-wide cosmetic sweep follow-up-after-merge |

### Final audit fix — FIN-01 closed (AC-8c Tier-1 launch probe)

The audit's one MUST-FIX-before-merge finding: the Tier-1 kiosk
launch-probe existed only as plan text (plan §4 phase-4 row + AC-8c).
Every earlier assertion greps launch-line TEXT out of served files;
nothing ever EXECUTED a generated launch command — so "a game actually
launches from pipeline output" was unproven at every automated tier.

| Finding | Disposition | What changed |
|---|---|---|
| FIN-01 MUST-FIX (final audit) — no implementation ever executes a generated `launch:` line; ADV-P6-05's accepted quote-shape findings were left suspended on this never-run empirical gate (D-P6e) | **fixed** (`tests/hosts/arcade-webapp-vm.nix`, this commit) | A probe shim named `jupiter-retroarch` (writeShellScriptBin; nothing real by that name exists in the test closure) is on PATH from boot via systemPackages + the smoke unit's own path, journals its FULL argv one %-q-escaped line per invocation to /tmp/launch-probe.log, exits 0. After the scrape→generate assertions the smoke gained a launch-probe block: read each SERVED metadata file's exact `launch:` line, substitute {file.path} exactly like the frontend — a real spaced ROM path RELATIVE TO THE METADATA FILE'S DIRECTORY (nes: `Crystal Carp (USA) (Rev A).nes`, snes: `Astral Almari (USA).sfc`) — then execute through `/bin/sh -c` EXACTLY as emitted. Assertions per system from the probe journal: argv[0]==jupiter-retroarch, `-L` + core present (fceumm / snes9x), final argv element == the substituted path, AND stat() resolves `<metadata dir>/<path>` to an existing regular file in the served tree. Any miss fails the whole smoke loudly. Negative control: with the final-argv assertion inverted, the run dies at the probe block with an explicit FAIL marker + make exit 1 (output captured below); restored → PASS |

**ADV-P6-05 acceptance is now empirically grounded.** D-P6e deferred the
quote-shape reconciliation to exactly this probe. The generator's emitted
quote bytes survived REAL shell execution byte-exact: multi-space,
parenthesized `(Rev A)` paths tokenized under sh parsing to precisely the
intended argv — had the quoting been wrong anywhere, word-splitting would
...
| D-P6e — quote byte-shapes coexist between Skyscraper compose and our generator; reconcile empirically at the AC-8c launch probe, NOT by unifying early | bare-quote values and backslash-escaped-quote values coexist across launcher-DB sources: Skyscraper's compose output and our generator (renderSystem's `launch:` line uses literal `"` bytes) can carry different BYTE shapes for the same quoting intent. Both tokenize identically per Pegasus CommandTokenizer semantics, so there is NO behavioral difference today and no served-file churn is justified — changing the generator's quote shape now would rewrite every metadata file byte-wise for zero functional gain and invalidate the golden fixtures (ADV-P6-05: informational only, no code change). The reconciliation is EMPIRICAL and deferred to the AC-8c launch probe: prove on a real kiosk that both shapes launch identically BEFORE europa cutover; only a observed divergence justifies touching the generator then → **evidence gate CLOSED 2026-08-23 (FIN-01)**: the AC-8c Tier-1 probe in the VM smoke now executes generated launch lines through real `/bin/sh -c` — the emitted bare-quote shape survived byte-exact with multi-space `(Rev A)` paths, argv recovered exactly as intended (see the FIN-01 closure section above); Tier-2 live-kiosk observation stays with the coordinator-gated europa/amalthea cutover | decided (deferred evidence gate) | `internal/generate/generate.go` renderSystem header/launch line + golden fixtures; plan §AC-8c CommandTokenizer reference; ADV-P6-05 reconciliation row above |
...
| D-P8d — europa webapp secret wiring restored pre-cutover (final-audit FIN-05) | the cutover retires rom-acquire/rom-scraper, whose modules DECLARED `jupiter_aria2_rpc_secret` / `screenscraper_creds` / `tgdb_apikey`; left as-wired (enable+openFirewall only), arcadeWebapp would lose aria2 RPC auth + both scraper credential sources at switch. Fix moves PATHS only, never values: `aria2SecretFile` points at the aria2 module's existing declaration (pure pointer per D-P2b); the two scraper keys exist unchanged in secrets/secrets.yaml and are re-declared verbatim host-side in europa's configuration.nix because the deleted rom-scraper.nix held their ONLY declarations. All three options wired — no null gap to own at cutover night | decided | hosts/europa/configuration.nix arcadeWebapp + sops blocks |

## Gauntlet scoreboard

**8 won / 0 remaining — gauntlet build phase complete (P1–P8)**. Exit (AC-10) = every piece P1–P8 won with the final
