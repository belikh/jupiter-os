...
- **Distinguishing responsibilities:**
  ...
  - Because that state sits on raw iSCSI zvols (which restic can't walk), `jupiter.services.stateBackup` lands an hourly `pg_dumpall` + Loki `rsync` on `europa:/tank/backups/elitedesk` (NFS, dataset name kept as-is — already provisioned), where europa's sanoid + restic snapshot it and ship it offsite — so the DB + logs are fully covered (see [06-storage-and-backups.md §8](06-storage-and-backups.md#8-diskless-host-state-backup-callisto--europa)).
