...
# self-destruct) via the BinaryLane API.
#
...
# Drive one full ephemeral build-server run: build the ISO, then hand off to
# scripts/binarylane-build-server.sh to create the BinaryLane server, boot
# it, and wait for it to rebuild the world and self-destruct.
# Requires BL_SIZE_SLUG, BL_REGION_SLUG and ISO_URL to be set (see that
# script's header) — this repo doesn't yet know where the built ISO should be
# hosted for BinaryLane to fetch it from (candidate: the existing B2 bucket).
