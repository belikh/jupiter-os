...
fleet MQTT broker, which now runs on callisto rather than amalthea). adrastea
is registered and CI-green but still on a placeholder disk and sops key —
physically installing it is a mechanical task, not a config one. This track
validated the incremental, cache-first approach and the CI flake-check
feedback loop.
